...
class BenchmarkTimer:
    ...
    def print_results(self, payload_size_kb):
        ...
        print(f"Avg Total UpdateTime: {self.total_flow_time / self.iterations:.6f} ms")
        print(f"Avg Identity Sign Time:   {self.get_avg('identity_sign'):.6f} ms (~45 B)")
        print(f"Avg Identity Vrfy Time:   {self.get_avg('identity_verify'):.6f} ms (~45 B)")
        print(f"Avg Payload Sign Time:    {self.get_avg('payload_sign'):.6f} ms (~150 B)")
        print(f"Avg Payload Vrfy Time:    {self.get_avg('payload_verify'):.6f} ms (~150 B)")
        print(f"Avg Encrypting Time: {self.get_avg('encrypt'):.6f} ms ({payload_size_kb:.1f}KB)")
        print(f"Avg Decrypting Time: {self.get_avg('decrypt'):.6f} ms ({payload_size_kb:.1f}KB)")
        print(f"Avg Hashing Time:    {self.get_avg('hash'):.6f} ms ({payload_size_kb:.1f}KB)")
